...
def random_scale_rotate_translate_3d_v2(tensor_img, tensor_labpse, tensor_lab, scale=0.3, rotate=45, translate=0.1, shear=0.1):
    '''
    The axis order of SimpleITK is x,y,z
    The axis order of numpy/tensor is z,y,x
    The arguments of all transformation should use the numpy/tensor order: [z,y,x]
    ...
    grid = F.affine_grid(theta, tensor_img.size(), align_corners=True).to(tensor_img.device)
    tensor_img = F.grid_sample(tensor_img, grid, mode='bilinear', padding_mode='zeros', align_corners=True)
    tensor_labpse = F.grid_sample(tensor_labpse.float(), grid, mode='nearest', padding_mode='zeros', align_corners=True).long()
    tensor_lab = F.grid_sample(tensor_lab.float(), grid, mode='nearest', padding_mode='zeros', align_corners=True).long()

    return tensor_img, tensor_labpse, tensor_lab
...
def crop_3d_v2(tensor_img, tensor_labpse, tensor_lab, crop_size, mode):
    assert mode in ['random', 'center'], "Invalid Mode, should be \'random\' or \'center\'"
    if isinstance(crop_size, int):
        crop_size = [crop_size] * 3
    ...
    cropped_img = tensor_img[:, :, rand_z:rand_z+crop_size[0], rand_y:rand_y+crop_size[1], rand_x:rand_x+crop_size[2]]
    cropped_lab = tensor_lab[:, :, rand_z:rand_z+crop_size[0], rand_y:rand_y+crop_size[1], rand_x:rand_x+crop_size[2]]
    crop_para = [rand_z, rand_y, rand_x]
    if tensor_labpse is not None:
        cropped_labpse = tensor_labpse[:, :, rand_z:rand_z+crop_size[0], rand_y:rand_y+crop_size[1], rand_x:rand_x+crop_size[2]]
        return cropped_img.contiguous(), cropped_labpse.contiguous(), cropped_lab.contiguous(), crop_para
    else:
        return cropped_img.contiguous(), None, cropped_lab.contiguous(), crop_para
    
def crop_3d_v3(tensor_img, tensor_lab, tensor_labori, crop_size, mode):
    assert mode in ['random', 'center'], "Invalid Mode, should be \'random\' or \'center\'"
    if isinstance(crop_size, int):
        crop_size = [crop_size] * 3
    ...
    cropped_img = tensor_img[:, :, rand_z:rand_z+crop_size[0], rand_y:rand_y+crop_size[1], rand_x:rand_x+crop_size[2]]
    cropped_lab = tensor_lab[:, :, rand_z:rand_z+crop_size[0], rand_y:rand_y+crop_size[1], rand_x:rand_x+crop_size[2]]
    cropped_labori = tensor_labori[:, :, rand_z:rand_z+crop_size[0], rand_y:rand_y+crop_size[1], rand_x:rand_x+crop_size[2]]
    crop_para = [rand_z, rand_y, rand_x]
    
    return cropped_img.contiguous(), cropped_lab.contiguous(), cropped_labori.contiguous(), crop_para

def crop_3d_para(tensor_lab, crop_para, crop_size):
    rand_z, rand_y, rand_x = crop_para
    cropped_lab = tensor_lab[:, :, rand_z:rand_z+crop_size[0], rand_y:rand_y+crop_size[1], rand_x:rand_x+crop_size[2]]
    
    return cropped_lab.contiguous()
...
def np_crop_3d_v2(np_img, np_labpse, np_lab, crop_size, mode):
    assert mode in ['random', 'center'], "Invalid Mode, should be \'random\' or \'center\'"
    if isinstance(crop_size, int):
        crop_size = [crop_size] * 3
    ...
    cropped_img = np_img[rand_z:rand_z+crop_size[0], rand_y:rand_y+crop_size[1], rand_x:rand_x+crop_size[2]]
    cropped_labpse = np_labpse[rand_z:rand_z+crop_size[0], rand_y:rand_y+crop_size[1], rand_x:rand_x+crop_size[2]]
    cropped_lab = np_lab[rand_z:rand_z+crop_size[0], rand_y:rand_y+crop_size[1], rand_x:rand_x+crop_size[2]]
    
    crop_para = [rand_z, rand_y, rand_x]

    return np.ascontiguousarray(cropped_img), np.ascontiguousarray(cropped_labpse), np.ascontiguousarray(cropped_lab), crop_para
